...
void send_food_grid(int fd) {
    // FOOD <row> <29chars of 0/1/2>
    char line[64];
    for (int y = 0; y < HEIGHT; y++) {
        char cells[WIDTH + 1];
        for (int x = 0; x < WIDTH; x++) {
            int v = food_check[y][x];
            cells[x] = (v == 2) ? '2' : (v == 1 ? '1' : '0');
        }
        cells[WIDTH] = '\0';
        int len = snprintf(line, sizeof(line), "FOOD %d %s\n", y, cells);
        ...
        if (FD_ISSET(listen_fd, &rfds)) {
            ...
            if (cfd >= 0) {
                ...
                if (stored) {
                    // ID 안내
                    int idx = -1;
                    for (int i = 0; i < MAX_CLIENTS; i++) if (clients[i].fd == cfd) idx = i;
                    if (idx >= 0) {
                        char id_buf[32];
                        int len = snprintf(id_buf, sizeof(id_buf), "ID %d\n", clients[idx].player_id);
                        send(cfd, id_buf, len, MSG_NOSIGNAL);
                    }
